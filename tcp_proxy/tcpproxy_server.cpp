...
// ALSO INCLUDED: https://github.com/svperbeast/struct
         ...
         if (!error)
         {
            async_write(downstream_socket_,
                 ...
                 boost::bind(&bridge::handle_downstream_write,
                      shared_from_this(),
                      boost::asio::placeholders::error));
            //printf("(Upstream read) Got %lu bytes", static_cast<unsigned long>(bytes_transferred));
         }
         else
            close();
      }
      ...
      */
                                  ...
                                  const size_t& bytes_transferred)
      {
         if (!error)
         {
            ...
            pWord = (char *)downstream_data_;
            std::string d = reinterpret_cast<char *>(pWord);
            std::string opcode = d.substr(0, 2);
            
            std::string pos = reinterpret_cast<char *>(pWord+4);
            //std::string X = pos.substr(0,4);
            char const *pos_x = pos.substr(0,4).data();
            char const *pos_x = substr(0,4).data();
            double X, Y, Z;
            struct_unpack(pos_x, "f", &X);
